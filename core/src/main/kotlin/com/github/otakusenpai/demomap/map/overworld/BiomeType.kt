...
enum class BiomeType constructor(val blocks: Boolean, val character: Char, val color: SColor, water: Boolean = false) {
    ...
    val darkColor: SColor
    val brightColor: SColor
    var water = false

    init {
        this.water = water
        this.darkColor = SColor(max((color.r - 20).toInt(), 0),
                max((color.g - 20).toInt(), 0), max((color.b - 20).toInt(), 0))
        this.brightColor = SColor(min((color.r + 20).toInt(), 255),
                min((color.g + 20).toInt(), 255), min((color.b + 20).toInt(), 255))
    }
